...
const STEPS = [
  { phase: "Frame problem", owner: "PM" },
  { phase: "Assess data & feasibility", owner: "DS + PM" },
  { phase: "Build & iterate", owner: "ML" },
  { phase: "Evaluate & sign off", owner: "ML + PM" },
  { phase: "Ship & integrate", owner: "Eng + MLOps" },
  { phase: "Monitor & retrain", owner: "Shared" },
] as const;

const ACCENT_CYCLE: DashboardAccent[] = ["blue", "amber", "purple", "emerald"];

export function MlTeamHandoffFlow() {
  return (
    <figure className="overflow-x-auto rounded-xl border border-border bg-muted/20 p-4 shadow-sm">
      <div className="flex min-w-[720px] items-stretch gap-1 py-2 md:min-w-0 md:flex-wrap md:justify-center">
        {STEPS.map((step, i) => {
          ...
          return (
            <div key={step.phase} className="flex items-center">
              ...
              >
                ...
                <span className="text-[10px] leading-snug text-muted-foreground">{step.owner}</span>
              </div>
              {i < STEPS.length - 1 && (
